...
export function updateCustomChainInContainer(
  ...
  if (updates.nativeCurrency) {
    ...
    try {
      ...
      if (error instanceof z.ZodError) {
        throw new Error(`Invalid native currency: ${error.issues.map((e) => e.message).join(", ")}`)
      }
      throw error
    }
  }
 ...
 */
export function removeCustomChainFromContainer(container: Container, chainId: number): void {
