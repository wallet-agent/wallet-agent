...
- `extract_wagmi_abi` - Extract ABI for a specific contract in JSON, TypeScript, or human-readable format
- `list_wagmi_functions` - List all callable functions for a contract with filtering by mutability
...
1. **Generate contracts with Wagmi CLI**:
   ```bash
   bunx @wagmi/cli generate
   ```

2. **Load contracts into Wallet Agent**:
   ```
   "Load wagmi config from ./src/generated.ts"
   ```
