...
describe("WalletEffects", () => {
	...
	beforeEach(() => {
		...
		mockChainAdapter = createMockChainAdapter([
			testChains.anvil,
			testChains.mainnet,
		]);
		privateKeyAddresses = [];
	...
	describe("connectWallet", () => {
		...
		it("throws error for invalid mock address", async () => {
			expect(
				walletEffects.connectWallet(testAddresses.privateKey1),
			).rejects.toThrow("is not in the list of mock accounts");
		});
		...
		it("connects to private key wallet", async () => {
			...
			const result = await walletEffects.connectWallet(
				testAddresses.privateKey1,
			);
	...
	describe("setWalletType", () => {
		...
		it("throws error when setting privateKey without imported keys", () => {
			expect(() => walletEffects.setWalletType("privateKey")).toThrow(
	...
	describe("signMessage", () => {
		...
		it("throws error when not connected", async () => {
			expect(walletEffects.signMessage("Hello")).rejects.toThrow(
				"No wallet connected",
			);
		});
	});
	...
	describe("switchChain", () => {
		...
		it("throws error for unsupported chain", async () => {
			expect(walletEffects.switchChain(999)).rejects.toThrow(
