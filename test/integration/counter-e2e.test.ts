...
describe.skipIf(!useRealAnvil)("Counter Contract E2E Tests", () => {
  ...
  beforeAll(async () => {
    ...
    if (!receipt.contractAddress) {
      throw new Error(
        "Counter contract deployment failed - no contract address",
      );
    }
...
`;
    ...
    const workingPath = join(
      process.cwd(),
      "test/contracts/wagmi-counter-deployed.ts",
    );
    ...
    const { text: incrementResult } = await expectToolSuccess(
      "send_transaction",
      {
        to: counterAddress,
        value: "0",
    ...
    const { text: decrementResult } = await expectToolSuccess(
      "send_transaction",
      {
        to: counterAddress,
        value: "0",
    ...
    console.log(
      "✅ Counter functions tested: incrementCounter (4x), decrementCounter (1x)",
    );
