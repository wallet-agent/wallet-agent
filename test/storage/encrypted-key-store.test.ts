...
describe("EncryptedKeyStoreManager", () => {
  ...
  describe("key management", () => {
    ...
    test("should return false when removing non-existent key", async () => {
      const removed = await keyStoreManager.removePrivateKey("0x1234567890123456789012345678901234567890" as Address)
      expect(removed).toBe(false)
    })

    test("should update key label", async () => {
      const address = await keyStoreManager.importPrivateKey(testPrivateKey, masterPassword, "Old Label")
  ...
  describe("password management", () => {
    ...
    test("should change master password", async () => {
      ...
      keyStoreManager.lock()
      await expect(keyStoreManager.unlock(masterPassword)).rejects.toThrow("Invalid master password")
    })
  ...
  describe("persistence", () => {
    test("should persist key store across manager instances", async () => {
      ...
      await keyStoreManager.createKeyStore(masterPassword)
      const address = await keyStoreManager.importPrivateKey(testPrivateKey, masterPassword, "Persistent Key")
