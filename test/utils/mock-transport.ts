...
export type MockResponses = Map<string, MockResponse | MockResponseFunction | unknown>

/**
 * Create a mock transport for testing
 * @param customResponses - Custom responses to override defaults
 */
export function createMockTransport(customResponses?: MockResponses): Transport {
  ...
  const allResponses = new Map([...defaultResponses, ...(customResponses || [])])
...
export function mockContractCall(
  ...
  return [
    ...
    ((params: unknown[]) => {
      ...
      if (
        (contractAddress === "*" || callData.to?.toLowerCase() === contractAddress.toLowerCase()) &&
...
export const MockPresets = {
  ...
  erc20Token: (decimals = 18): MockResponses =>
    new Map([
      ...
      mockContractCall("*", "0x313ce567", `0x${decimals.toString(16).padStart(64, "0")}`), // decimals()
