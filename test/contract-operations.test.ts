...
describe("Contract Operations", () => {
  ...
  describe("writeContract", () => {
    test("throws error when no wallet connected", async () => {
      ...
      await expect(writeContract(params)).rejects.toThrow(
        "No wallet connected",
      );
    });
    ...
    test("handles private key wallet type", async () => {
      ...
      const { importPrivateKey, setWalletType } = await import(
        "../src/wallet-manager"
      );
  ...
  describe("integration scenarios", () => {
    ...
    test("handles contract operations with different wallet states", async () => {
      ...
      await expect(writeContract(writeParams)).rejects.toThrow(
        "No wallet connected",
      );
    });
  });
